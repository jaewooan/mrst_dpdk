...
% fracture
perm_fracture = 1*milli*darcy*ones(G.cartDims); 
rock_fracture = struct('perm', reshape(perm_fracture, [], 1), ...
              'poro', ones(G.cells.num, 1)*0.375,... 
              'vol_fraction', ones(G.cells.num, 1)*0.5); 
               % note, poro = intrinsic_poro*vol_fraction, therefore, for
               % this case intrinsic_poro is 0.6

% matrix
perm_matrix = 1*milli*darcy*ones(G.cartDims); 
...
fluid = initSimpleADIFluid('phases', 'W', 'mu', 0.0981*centi*poise, 'rho', ...
...
for i=1:nx+1
    ...
    R__(i,:) = u((nx+1)*2*(i-1)+1:2:(nx+1)*2*i,60);
end
pcolor(X__, Y__, R__);
set(gca,'YDir','normal') 
colorbar
...
for i=1:11
    X__(i, :) =  G.nodes.coords((nx+1)*(i-1)+1:(nx+1)*i,1)';
    Y__(i, :) =  G.nodes.coords((nx+1)*(i-1)+1:(nx+1)*i,2)';
    R__(i, :) = u((nx+1)*2*(i-1)+2:2:(nx+1)*2*i+1,60);
end
pcolor(X__, Y__, R__);
set(gca,'YDir','normal') 
colorbar