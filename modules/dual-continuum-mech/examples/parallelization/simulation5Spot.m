...
function simulation5SPot(iParallel)    
    ...
    load(filename);
    results=zeros(0,0);
    for i=1:nCase/nParallel % nCase = 10000, nx = 100, ny = 100
        %% Setup default options
        %E_m = reshape(EM_(:,i), nx*ny, 1);
        %E_f = reshape(EF_(:,i), nx*ny, 1);
        perm_matrix = reshape(perm_m_(:,i), nx, ny)*milli*darcy;
        perm_fracture = reshape(perm_f_(:,i), nx, ny)*milli*darcy;
        ...
        mech = struct('E', E, 'nu', nu, 'E_m', E_m, 'nu_m', nu_m, 'E_f', E_f, 'nu_f', nu_f,...
                      'K_s', K_s, 'el_bc', el_bc, 'load', load_);
