classdef GeothermalModel < ReservoirModel & GenericReservoirModel
    ...
    methods
        ...
        function model = setupStateFunctionGroupings(model, varargin)
            ...
            fp = fp.setStateFunction('PhaseInternalEnergy', PhaseInternalEnergy(model));
            fp = fp.setStateFunction('RockInternalEnergy' , RockInternalEnergy(model));
            % Thermal energy in fluid phases
            fp = fp.setStateFunction('PhaseMass'         , PhaseMass(model));
            fp = fp.setStateFunction('PhaseThermalEnergy', PhaseThermalEnergy(model));
            % Thermal energy in rock
            fp = fp.setStateFunction('RockThermalEnergy', RockThermalEnergy(model));
            % Total thermal energy
            fp = fp.setStateFunction('TotalThermalEnergy', TotalThermalEnergy(model));
            % Replace
