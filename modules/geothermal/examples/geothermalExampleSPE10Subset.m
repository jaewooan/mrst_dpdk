...
S  = sparse(find(ic), 1:nnz(ic), 1, model.G.faces.num, nnz(ic));
vAdv  = cellfun(@(st) sqrt(sum(faceFlux2cellVelocity(G, S*st.heatFluxAdv ).^2,2)), states, 'UniformOutput', false);
vCond = cellfun(@(st) sqrt(sum(faceFlux2cellVelocity(G, S*st.heatFluxCond).^2,2)), states, 'UniformOutput', false);
...
ic = model.operators.internalConn;
S  = sparse(find(ic), 1:nnz(ic), 1, model.G.faces.num, nnz(ic));
vAdvBC  = cellfun(@(st) sqrt(sum(faceFlux2cellVelocity(G, S*st.heatFluxAdv ).^2,2)), statesBC, 'UniformOutput', false);
vCondBC = cellfun(@(st) sqrt(sum(faceFlux2cellVelocity(G, S*st.heatFluxCond).^2,2)), statesBC, 'UniformOutput', false);
