function src = addThermalSource(src, c, v, varargin)
...
%
% PARAMETERS:
%   src    - Source structure from a prior call to 'addThermalSource' which will
...
%            Temperature (typically in units of K). 
...
%          - v:    Values of these explicit sources
...
%
% SEE ALSO:
%   `addWell`, `addThermalBC`, `solveIncompFlow`.
...
if isempty(src),
   src = struct('cell', [], 'value', [], 'sat', []);
end

if numel(v) == 1
    v = repmat(v, numel(c), 1);
end

assert (numel(c) == numel(v), ['The number of values should equal the number', ...
